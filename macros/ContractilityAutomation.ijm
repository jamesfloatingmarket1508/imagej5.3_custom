...
macro "Contractility" {
	...
	if (slices<2)
		print(" WRONG INPUT ALERT... This macro requires a stack");
	if (slices<20)
		print(" WRONG INPUT ALERT... This macro requires a stack of at least 20 slices");
	if (channels!=1)
		print(" WRONG INPUT ALERT... This macro doesn't work with multiple-channel images");
	...
	if (frame_interval<=0)
		print(" WRONG INPUT ALERT... Invalid frame interval: must be positive");
	if (nwin == 0) {
		if (wind_length<=0)
			print(" WRONG INPUT ALERT... Invalid window length: must be positive");
	...
	if (use_blipper) {
		...
		if (nblips==0)
			print(" WRONG INPUT ALERT... No blips detected");
	...
	if (use_blipper) {
		...
		if (nevents<2)
			print(" WRONG INPUT ALERT... Need at least 2 peaks in 1st derivative");
	...
	if (nevents<2)
		print(" WRONG INPUT ALERT... Need at least two transients");
	...
	if (average_mode>0 && ntransients>1) {
		...
		navg = minpre + minpost;
		if (navg<5)
			print(" WRONG INPUT ALERT... Average has fewer than 5 points");
		xavg = newArray(navg);
		yavg = newArray(navg);
...
function findPercentLevelCrossing(x, y, pc, kdir, i0) {
	if((pc<1)||(pc>99))
		print(" WRONG INPUT ALERT... Percent level must be in range 1-99");
	...
	if (found==0)
		print(" WRONG INPUT ALERT... findPercentLevelCrossing failed");
...
function getBlips(thresholdpc) {
	...
	if (max-min < MIN_BLIP_AMPLITUDE)
		print(" WRONG INPUT ALERT... No blips detected (too small amplitude)!");
...
function checkBlipGoodness(blips, frame_interval) {
	n = blips.length;
	if (n == 0)
		print(" WRONG INPUT ALERT... No blips detected (peak detection failed)!");
	else if (n == 1)
		print(" WRONG INPUT ALERT... Only one blip detected!");
	else {
...
function Assert(cond, msg) {
	if (cond==0)
		print(" WRONG INPUT ALERT... Assertion failed: " + msg);
	}
